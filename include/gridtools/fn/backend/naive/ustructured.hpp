...
namespace gridtools::fn {
    namespace unstructured_naive_impl_ {
        ...
        struct iterator_maker {
            GT_FUNCTION consteval auto operator()() const {
                return [](auto tag, auto &&ptr, auto const &strides) {
                    return neighbor_iter(Horizontal(), *at_key<pos_t>(ptr), strided_iter(tag, ptr, strides));
                };
            }
        };
    ...
    template <class Stencil, class Sizes, class Offsets, class Horizontal, class Output, class Inputs>
    void fn_apply(
        naive, unstructured<Sizes, Offsets, Horizontal> const &domain, Stencil, Output &output, Inputs inputs) {
        using namespace unstructured_naive_impl_;
        pos_t pos;
        auto out = sid::rename_dimensions<Horizontal, unstructured_naive_impl_::hor>(output);
        naive_apply<Stencil, unstructured_naive_impl_::iterator_maker<Horizontal>>(
